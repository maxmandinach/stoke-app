-- Migration: Shared Content Model Architecture
...
UPDATE content SET 
    duration_hours = 1.0,
    quick_summary = COALESCE(summary, ''),
    full_summary = COALESCE(summary, ''),
...
SELECT 
    ...
    COALESCE(SUM(ucl.total_sessions), 0) as total_sessions,
    COALESCE(SUM(ucl.total_time_minutes), 0) as total_learning_time_minutes,
    COALESCE(SUM(ucl.total_questions_answered), 0) as total_questions_answered,
    CASE 
        WHEN SUM(ucl.total_questions_answered) > 0 
        THEN ROUND((SUM(ucl.total_questions_correct)::DECIMAL / SUM(ucl.total_questions_answered)) * 100, 1)
        ELSE 0
    END as overall_accuracy_rate,
    
    -- Progress metrics
    ROUND(AVG(ucl.mastery_percentage), 1) as average_mastery_percentage,
    COALESCE(SUM(ucl.questions_due_count), 0) as questions_due_today,
    
    -- Recent activity
    MAX(ucl.last_accessed_at) as last_session_at,
    COUNT(CASE WHEN ucl.last_accessed_at >= NOW() - INTERVAL '7 days' THEN 1 END) as sessions_this_week
...
SELECT 
    ...
    COUNT(DISTINCT ucl.content_id) as content_count,
    COALESCE(SUM(c.total_questions), 0) as total_questions,
    ...
    ROUND(AVG(ucl.mastery_percentage), 1) as average_mastery_percentage,
    COALESCE(SUM(ucl.total_time_minutes), 0) as total_time_spent_minutes,
    
    -- Recent activity
    MAX(ucl.last_accessed_at) as last_reviewed_at
    
FROM topics t
JOIN episode_topics et ON t.id = et.topic_id
JOIN user_content_library ucl ON et.content_id = ucl.content_id
JOIN content c ON ucl.content_id = c.id
WHERE t.is_active = TRUE
GROUP BY t.id, ucl.user_id, t.name;
