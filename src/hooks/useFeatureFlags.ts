...
export const useFeatureFlags = (): FeatureFlags => {
  ...
  const envUseNewDesign = process.env.NEXT_PUBLIC_USE_NEW_DESIGN === 'true';
...
export const toggleFeatureFlag = (flag: keyof FeatureFlags, value?: boolean): void => {
  ...
  switch (flag) {
    case 'useNewDesign':
      const newValue = value !== undefined ? value : !JSON.parse(localStorage.getItem('stoke_dev_use_new_design') || 'false');
